...
void BOARD::Flip(POS p,FIN f) {
	if(f==FIN_X) {  // 如果沒給翻出來的子，就自己random一個結果
		int i,sum=0;
		for(i=0;i<14;i++) // 計算尚未翻出的子數
			sum+=cnt[i];
		sum = rand()%sum;
		for(i=0;i<14;i++)
			if((sum-=cnt[i])<0)
...
void BOARD::DoMove(MOV m, FIN f) {
    ...
    if (m.ed!=m.st) {
    	if(fin[m.ed] != FIN_E){ // if m.ed isn't empty.
			...
			zob_key ^= zob[fin[m.ed]][m.ed]; // 拿掉被吃得子
		}
		
		zob_key ^= zob[fin[m.st]][m.st]; // 拿掉要移動的子
		zob_key ^= zob[fin[m.st]][m.ed]; // 放在目標位置
		fin[m.ed]=fin[m.st];
		fin[m.st]=FIN_E;
		who^=1;
    } else {
